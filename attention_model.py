...
from gsm_model import VariationalTopicModel  
...
class Vanilla_attention_model(object):
    ...
    def __init__(self, num_classes=20, pretrained_embed=None, embedding_size=100, 
        ...
        with tf.variable_scope("doc2vec"):
            ...
            #doc_encoded = self.BidirectionalLSTMEncoder(self.embedded_input, self.hidden_size, name='bi-lstm')
        ...
        self.predict_prob = tf.nn.softmax(out)
    ...
    def BidirectionalLSTMEncoder(self, inputs, units, name):
        with tf.variable_scope(name):
            LSTM_cell_fw = rnn.LSTMCell(units)
            LSTM_cell_bw = rnn.LSTMCell(units)
